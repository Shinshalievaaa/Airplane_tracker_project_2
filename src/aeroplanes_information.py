class Aeroplane:
    __slots__ = ('ID', 'country', 'velocity', 'altitude')
    ...
    def __verify_data(cls, other):
        if not isinstance(other, (float, int, Aeroplane)):
            raise TypeError(
                "Операнд справа должен иметь тип float или int или Aeroplane"
            )
    ...
    def get_aeroplanes_by_altitude(
        filtered_aeroplanes: list[dict], altitude_range: list
    ) -> list[dict]:
        ...
        for item in filtered_aeroplanes:
            ...
            if isinstance(item.get("altitude"), (float, int)):
                if (
                    item.get("altitude") >= range_min
                    and item.get("altitude") <= range_max
                ):
